#include <fstream>

#include "color.h"
#include "ray.h"
#include "hittable_list.h"
#include "utility.h"
#include "sphere.h"
#include "camera.h"
#include "material.h"
#include "bvh.h"



hittable_list random_scene() {
    hittable_list world;

    auto ground_material = make_shared<lambertian>(color(0.5, 0.5, 0.5));
    world.add(make_shared<sphere>(point3(0, -1000, 0), 1000, ground_material));

    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            auto choose_mat = random_double();
            point3 center(a + 0.9 * random_double(), 0.2, b + 0.9 * random_double());

            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                shared_ptr<material> sphere_material;

                if (choose_mat < 0.8) {
                    // diffuse
                    auto albedo = color::random() * color::random();
                    sphere_material = make_shared<lambertian>(albedo);
                    world.add(make_shared<sphere>(center, 0.2, sphere_material));
                }
                else if (choose_mat < 0.95) {
                    // metal
                    auto albedo = color::random(0.5, 1);
                    auto fuzz = random_double(0, 0.5);
                    sphere_material = make_shared<metal>(albedo, fuzz);
                    world.add(make_shared<sphere>(center, 0.2, sphere_material));
                }
                else {
                    // glass
                    sphere_material = make_shared<dielectric>(1.5);
                    world.add(make_shared<sphere>(center, 0.2, sphere_material));
                }
            }
        }
    }

    auto material1 = make_shared<dielectric>(1.5);
    world.add(make_shared<sphere>(point3(0, 1, 0), 1.0, material1));

    auto material2 = make_shared<lambertian>(color(0.4, 0.2, 0.1));
    world.add(make_shared<sphere>(point3(-4, 1, 0), 1.0, material2));

    auto material3 = make_shared<metal>(color(0.7, 0.6, 0.5), 0.0);
    world.add(make_shared<sphere>(point3(4, 1, 0), 1.0, material3));

    return world;
}



color ray_color(const ray& r, const hittable& world, int depth) {
    hit_record rec;

    // If we've exceeded the ray bounce limit, no more light is gathered.
    if (depth <= 0)
        return color(0, 0, 0);

    if (world.hit(r, 0.001, infinity, rec)) {
        ray scattered;
        color attenuation;
        if (rec.mat_ptr->scatter(r, rec, attenuation, scattered))
            return attenuation * ray_color(scattered, world, depth - 1);
        return color(0, 0, 0);
    }
    vec3 unit_direction = unit_vector(r.direction());
    auto t = 0.5 * (unit_direction.y() + 1.0);
    return (1.0 - t) * color(1.0, 1.0, 1.0) + t * color(0.0, 0.7, 1.0);
    //return color(0.5, 0.0, 0.0);
}



int main(void) {

    // Image
    constexpr double aspect_ratio = 16.0 / 9.0;
    constexpr int image_width = 400;
    constexpr int image_height = static_cast<int>(image_width / aspect_ratio);
    constexpr int samples_per_pixel = 50;
    const int max_depth = 50;


    // World
    hittable_list world = random_scene();
    //hittable_list world;
    //auto red = make_shared<lambertian>(color(1.0, 0.0, 0.0));
    //auto yellow = make_shared<lambertian>(color(1.0, 1.0, 0.0));
    //auto grey = make_shared<lambertian>(color(0.5, 0.5, 0.5));
    //auto blue = make_shared<lambertian>(color(0.0, 0.0, 1.0));
    //auto mirror = make_shared<metal>(color(0.5, 0.3, 0.2), 0.1);
    //auto glass = make_shared<dielectric>(1.5);
    // world.add(make_shared<cuboid>(point3(0.0, 1.0, 0.0), 1.0, 2.0, 3.0, mirror));
    //world.add(make_shared<sphere>(point3(0, -1000, 0), 1000, grey));
    //world.add(make_shared<sphere>(point3(1.5, 1, -1.5), 1.0, red));
    //world.add(make_shared<sphere>(point3(-0.3, 3.5, 0), 1.5, yellow));
    //world.add(make_shared<sphere>(point3(0, 0.3, 3), 0.3, blue));
    //auto material2 = make_shared<lambertian>(color(1.0, 1.0, 0.0));
    //world.add(make_shared<sphere>(point3(1, 1, 0), 1.0, material2));
    //world.add(make_shared<sphere>(point3(0.0, 1.0, 0.0), 1.0, material1));


    // Camera
    point3 lookfrom(13, 2, 3);
    point3 lookat(0, 0, 0);
    vec3 vup(0, 1, 0);
    auto dist_to_focus = 10.0;
    auto aperture = 0.1;
    double time0 = 0.0;
    double time1 = 0.0;

    camera cam(lookfrom, lookat, vup, 20, aspect_ratio, aperture, dist_to_focus, time0, time1);

    // BVH
    bvh_node world_bvh(world, time0, time1);


    // Render
    std::ofstream os("image.ppm");
    os << "P3\n" << image_width << " " << image_height << "\n255\n";

    for (int r = image_height - 1; r >= 0; --r) {
        std::cout << "\rScanlines remaining: " << r << ' ' << std::flush;
        for (int c = 0; c < image_width; ++c) {

            // Anti-Aliasing with random sampling
            color pixel_color(0, 0, 0);
            for (int s = 0; s < samples_per_pixel; ++s) {
                auto u = (c + random_double()) / (image_width - 1);
                auto v = (r + random_double()) / (image_height - 1);
                ray ra = cam.get_ray(u, v);
                pixel_color += ray_color(ra, world_bvh, max_depth);
            }  
            write_color(os, pixel_color, samples_per_pixel);

            // Anti-Aliasing with regular grid
            //const int samples_per_axis = 70;
            //const double delta = 1.0 / samples_per_axis;
            //color pixel_color(0, 0, 0);
            //for (int aar = 0; aar < samples_per_axis; ++aar) {
            //    for (int aac = 0; aac < samples_per_axis; ++aac) {
            //        const double u = (c - 0.5 + delta / 2 + delta * aac) / (image_width - 1);
            //        const double v = (r - 0.5 + delta / 2 + delta * aar) / (image_height - 1);
            //        ray ra = cam.get_ray(u, v);
            //        pixel_color += ray_color(ra, world, max_depth);
            //    }
            //}
            //write_color(os, pixel_color, samples_per_axis * samples_per_axis);
        }
    }

    std::cout << "\nDone.\n";
}

